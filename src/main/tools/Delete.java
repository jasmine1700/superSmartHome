...
//			pstmt.setString(4,str);
			...
			if(table.equals("FAMILY")){
				...
				sql1 = "update temphumi set th_isDeleted = 1 where exists(select * from device where d_deviceId= " + str +")";
				sql2 = "update light set l_isDeleted = 1 where exists(select * from device where d_deviceId= " + str + ")";
				sql3 = "update door set do_isDeleted = 1 where exists(select * from device where d_deviceId= " + str + ")";
				stmt.executeUpdate(sql1);
			...
			JOptionPane.showMessageDialog(null, "Fail query. ", "Fail", JOptionPane.WARNING_MESSAGE);
		}
