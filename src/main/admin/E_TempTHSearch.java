...
public class E_TempTHSearch extends JFrame implements ActionListener{
	...
	public E_TempTHSearch(String manuId,int flag) {
		...
		if(flag == 0){
			...
			this.setTitle("Manu-ViewData-TempHumi-TemperatureSearch");
		}
			...
			public void actionPerformed(ActionEvent e) {
				if(textField.getText().equals("") || textField_1.getText().equals("")) {
					...
					try {                                                                              
				        ...
				        ResultSet rs = null;
				        if(flag == 0) {
				        	rs = stmt.executeQuery("SELECT * FROM temphumi WHERE th_temperature BETWEEN " + textField.getText() + " AND " + textField_1.getText());				        
				        }
				        else {
				        	rs = stmt.executeQuery("SELECT * FROM temphumi,device WHERE th_temperature BETWEEN " + textField.getText() + " AND " + textField_1.getText() + " AND d_manuId = \"" + manuId + "\" AND th_deviceId = d_deviceId;");				        
				        }
